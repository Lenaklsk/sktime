...
def create_mock_data(n_samples=10, hierarchy=True):
    ...
    if hierarchy:
        ...
        data = [
            (x, y, date, np.random.uniform(10, 20000))
        ...
        df_total = (
            df.groupby(["X", "date"], as_index=False)["value"].sum().assign(Y="__total")
        )
...
def test_recursive_reduction_forecaster_fit():
    ...
    assert hasattr(forecaster, "_is_fitted"), (
        "Forecaster is missing the '_is_fitted' attribute"
    )
...
def test_recursive_reduction_forecaster_pooling():
    ...
    for pooling in ["local", "global"]:
        forecaster = RecursiveReductionForecaster(
            estimator=estimator, window_length=5, pooling=pooling
        )
        forecaster.fit(df)
        y_pred = forecaster.predict(fh=fh)
...
def test_recursive_reduction_forecaster_hierarchy():
    ...
    assert y_pred.shape[0] == len(fh) * unique_combinations, (
        "Not all hierarchy nodes were predicted"
    )

    unique_predictions = y_pred.reset_index().groupby(["X", "Y"])["value"].nunique()
    assert (unique_predictions > 1).any(), (
        "All hierarchy nodes have identical predictions, which is unexpected"
    )

    # Ensure each node has its own model
    if hasattr(forecaster, "models_"):
        assert len(forecaster.models_) == unique_combinations, (
            "Each node should have a separate model"
        )
...
def test_recursive_reduction_forecaster_global_one_level():
    ...
    forecaster = RecursiveReductionForecaster(
        estimator=LinearRegression(), window_length=5, pooling="global"
    )
    forecaster.fit(df)
    y_pred = forecaster.predict(fh=fh)
    ...
    assert forecaster.pooling == "local", (
        "Pooling should be automatically set to 'local'"
    )
    assert isinstance(y_pred, (pd.Series, pd.DataFrame)), (
        "Predictions should be a pandas object"
    )
